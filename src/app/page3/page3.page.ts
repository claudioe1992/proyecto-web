...
export class Page3Page implements OnInit {
  ...
  // Método para redirigir a la pagina Ver puntuacion
  navigateToScores() {
    console.log("Redirigiendo a la página de error");
    this.navCtrl.navigateForward('/scores'); // Navega a la pagina Ver puntuacion
  }


  navigateToGame() {
    console.log("Comenzar partida clickeado");
    this.audioService.playSound('assets/sounds/magia.mp3').then(() => {
      console.log("Reproducción de sonido iniciada");
    }).catch(err => {
      console.error("Error al reproducir sonido:", err);
    });
    this.navCtrl.navigateForward('/juego'); // Navega a la página de terror
  }
}
